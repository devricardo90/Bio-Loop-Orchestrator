...
- Em 2026-05-22, `UI-01B3 - Seller/Admin/Billing Screens Visual Baseline` foi concluido em producao: commit 5b9a1f2, visual baseline aplicado a seller lots/results, seller reports/billing, admin buyers e admin disputes sem alterar auth, CSRF, API calls, mutations, route guard, IDs ou backend.
- Em 2026-05-22, `UI-01C - Mobile and Product Copy Polish` entrou em REVIEW local: polish responsivo aplicado ao header/nav/CTAs/mobile hero, role badges encurtados para linguagem de workspace, termos tecnicos visiveis convertidos para linguagem de produto, e assertions web/e2e alinhadas ao novo copy. Auth, rotas, credenciais demo, backend, API, DB, env, deploy e README nao foram alterados. Commit pendente de autorizacao do Trigger.
- Em 2026-05-22, `UI-01D - Login Desktop Layout Polish` entrou em REVIEW local: login compartilhado ganhou uma composicao desktop mais larga em duas colunas, painel contextual secundario e skip action integrada como link discreto. Tabs Buyer/Seller/Admin, credenciais demo, auth/session/redirects, backend, API, DB, env, deploy e README nao foram alterados. Typecheck, lint e test PASS. Commit pendente de autorizacao.
- Em 2026-05-22, `CI-01D - Stabilize Browser E2E after UI-01C Copy Polish` entrou em REVIEW local: 6 assertions E2E alinhadas ao copy humanizado de UI-01C. buyer-real-data.e2e.spec.ts linhas 39-41 e 53 passaram a usar `formatEnumLabel(pickupStatus)` em vez do enum raw. admin.e2e.spec.ts substituiu PENDING/APPROVED/OPEN/RESOLVED por Pending/Approved/Open/Resolved. `formatEnumLabel` adicionado a helpers.ts. Typecheck, lint e test PASS. Browser E2E nao rodou localmente (stack 3101/4101 down). Commit pendente de autorizacao.
- Em 2026-05-22, `AUTH-01B - Stabilize Demo Auth, E2E Login, and Role-Based Fetch Noise` entrou em REVIEW local: helper Browser E2E atualizado para tabs `buyer`/`seller`/`admin`, assertion de login expirada alinhada ao copy atual, fetch global de buyer feed limitado a sessao buyer e demo local documentada como Web `http://localhost:3002` + API `http://localhost:4000`. README e UI polish fino seguem bloqueados ate commit/autorizacao desta correcao.
